...
const createFBO = (width, height, initData) => {
    const framebuffer = gl.createFramebuffer();
    gl.bindFramebuffer(gl.FRAMEBUFFER, framebuffer);
    const texture = gl.createTexture();
    gl.bindTexture(gl.TEXTURE_2D, texture);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA32F, width, height, 0, gl.RGBA, gl.FLOAT, initData !== null && initData !== void 0 ? initData : null); // TODO Revisit formats
...
let positionsFBO = createFBO(numParticles, 1, new Float32Array(new Array(numParticles)
    .fill(0)
    .map((v, i) => [50.0 + Math.random() * 5.0, 0.0 + i * 1.01, 0.0, 0.0])
    .flat()));
let newPositionsFBO = createFBO(numParticles, 1);
const collisionProgram = createProgram(fullScreenVertexShader, compileShader(
/* GLSL */ `
...
`, gl.FRAGMENT_SHADER));
const positionProgram = createProgram(fullScreenVertexShader, compileShader(
/* GLSL */ `
precision mediump float;
...
const computeCollisions = () => {
    ...
    const fullScreenVertexAttribLoc = gl.getAttribLocation(collisionProgram, "position");
...
const computePositions = () => {
    ...
    gl.bufferData(gl.ARRAY_BUFFER, fullScreenVertexBufferData, gl.STATIC_DRAW);
    // TODO Only need to run this once or after bufferData each time?
    const fullScreenVertexAttribLoc = gl.getAttribLocation(positionProgram, "position");
    gl.enableVertexAttribArray(fullScreenVertexAttribLoc);
    gl.vertexAttribPointer(fullScreenVertexAttribLoc, 2, gl.FLOAT, false, 0, 0);
    gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
...
//
//
const collisionOutputCanvas = document.getElementById("collision_output");
...
const outputPositions = () => {
    ...
    for (let i = 0; i < numParticles; i++) {
        ...
        displayCtx.arc(positionsOutput[i * 4], 100.0 - positionsOutput[i * 4 + 1], particleSize, 0, Math.PI * 2);
...
const outputCollisions = () => {
    ...
    gl.readPixels(0, 0, numParticles, numParticles, gl.RGBA, gl.FLOAT, collisionsOutput);
    // console.log(collisionsOutput);
    collisionOutputCtx === null || collisionOutputCtx === void 0 ? void 0 : collisionOutputCtx.putImageData(new ImageData(new Uint8ClampedArray([...collisionsOutput]
        .map((v) => Math.floor(v * 100) + 127)
        .map((v, i) => (i % 4 === 3 ? 255 : v))), numParticles), 0, 0);
};
const mainLoop = () => {
    computeCollisions();
    computePositions();
